...
public class DebugNewProcessSequence extends ReflectionSequence {
	...
	public DebugNewProcessSequence(DsfExecutor executor, DsfSession session, boolean isInitial, IDMContext dmc, String file, Map<String, Object> attributes, DataRequestMonitor<IDMContext> rm) {
		super(executor, rm);
		System.out.println("mjs debug:enter DebugNewProcessSequence\n");
		fContext = dmc;
		fBinaryName = file;
	...
	public void stepRemoteConnection(RequestMonitor rm) {
		...
		if (fBackend.getSessionType() == SessionType.REMOTE && !fBackend.getIsAttachSession()) {
			...
			final DataRequestMonitor dataRequestMonitor = new DataRequestMonitor<String[]>(ImmediateExecutor.getInstance(), rm) {
    				@Override
    				protected void handleSuccess() {
					System.out.println("mjs debug:enter handleSuccess Attach\n");
			...
			System.out.println("mjs debug:2222 stepRemoteConnection Attach\n");
			IConnectHandler connectCommand = (IConnectHandler)fSession.getModelAdapter(IConnectHandler.class);
			System.out.println("mjs debug:2222 connectCommand" + connectCommand.toString() + "\n");
			if (connectCommand instanceof IConnect) {
				((IConnect)connectCommand).connect(dataRequestMonitor);
			} else {
				rm.done();
			}
