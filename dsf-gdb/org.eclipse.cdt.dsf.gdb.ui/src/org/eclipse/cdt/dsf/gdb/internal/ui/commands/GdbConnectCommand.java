...
import org.eclipse.cdt.dsf.gdb.service.PDBRemoteCommand;
...
public class GdbConnectCommand extends RefreshableDebugCommand implements IConnectHandler, IConnect {
    ...
    protected class PromptForPidJob extends UIJob {
		...
		System.out.println("mjs debug enter runInUIThread\n");
	...
	System.out.println("mjs debug enter getAllDebuggedProcesses\n");
				...
				if (!sessions.isEmpty()) {
					...
					nextSession.getExecutor().execute(new DsfRunnable() {
						...
						public void run() {
							...
							//System.out.println("mjs debug nextProcService:" + nextProcService.getProcessesBeingDebugged + "\n");
    ...
    {
	System.out.println("mjs debug enter connect\n");
    	fExecutor.execute(new DsfRunnable() {
            @Override
    		public void run() {
    			...
    			IGDBBackend backend = fTracker.getService(IGDBBackend.class);
                	System.out.println("mjs debug connect:" + procService + " " + commandControl + " " + backend + "\n");	
    			if (procService != null && commandControl != null && backend != null) {
        			final ICommandControlDMContext controlCtx = commandControl.getContext();
        			//System.out.println("mjs debug connect method:" + procService.getRunningProcesses + "\n");	
										...
										protected void handleSuccess() {
											getAllDebuggedProcesses(controlCtx, true, new ImmediateDataRequestMonitor<List<String>>(rm) {
												...
												protected void handleSuccess() {
													...
													System.out.println("mjs debug dbgPids:" + dbgPids.toString() + "\n");
...
System.out.println("mjs debug call attachToProcesses\n");																		attachToProcesses(controlCtx, (IProcessExtendedInfo[])data, rm);
	...
	System.out.println("mjs debug attachToProcesses:" + procService + "\n");
	System.out.println("mjs debug backend:" + backend + "\n");
    			...
    			protected void handleCompleted() {
    				...
    				if (!procList.isEmpty()) {
    					...
    					procService.isDebuggerAttachSupported(controlDmc, new ImmediateDataRequestMonitor<Boolean>() {
    						@Override
    						protected void handleCompleted() {
							System.out.println("mjs debug AttachToProcessRequestMonitor handleCompleted\n");						
							System.out.println("debug isSuccess:" + isSuccess() + "\n");
							System.out.println("debug getData:" + getData() + "\n");
								...
								System.out.println("debug pidStr:" + pidStr + "\n");
    								if (backend.getSessionType() == SessionType.REMOTE) {
									System.out.println("debug enter remote\n");
									new PDBRemoteCommand().attachToRemoteProcess(pidStr);
								...
								if (backend.getSessionType() == SessionType.REMOTE) {
							    			...
							    			System.out.println("debug pidStr:" + pidStr + "\n");
							    			new PDBRemoteCommand().attachToRemoteProcess(pidStr);
							    			System.out.println("end debug pidStr:" + pidStr + "\n");
								}
    								rm.done();
    							}
    						}
